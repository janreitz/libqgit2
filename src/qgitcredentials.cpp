...
protected:
    int create(git_credential **cred, const char*, const char*, unsigned int allowedTypes)
    {
        if (allowedTypes & GIT_CREDTYPE_USERNAME) {
            return git_credential_username_new(cred, m_user_name.data());
        }

        if (allowedTypes & GIT_CREDTYPE_SSH_KEY) {
            return git_credential_ssh_key_new(cred, m_user_name.data(), m_public_key_path.data(), m_private_key_path.data(), m_passphrase.data());
        }
